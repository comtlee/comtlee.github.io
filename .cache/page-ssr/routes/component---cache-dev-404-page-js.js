...
class Dev404Page extends (react__WEBPACK_IMPORTED_MODULE_0___default().Component) {
  constructor(props) {
    ...
    const urlState = query_string__WEBPACK_IMPORTED_MODULE_2__["default"].parse(location.search);
  ...
  setSearchUrl(searchValue) {
    ...
    const searchMap = query_string__WEBPACK_IMPORTED_MODULE_2__["default"].parse(search);
    searchMap.filter = searchValue;
    const newSearch = query_string__WEBPACK_IMPORTED_MODULE_2__["default"].stringify(searchMap);
    if (search !== `?${newSearch}`) {
